...
- 作为学者, 伯特兰的主要贡献在微分几何, 数论, 热力学等领域. 今天伯特兰仍然为学界所熟知, 主要是因为以他命名的两个悖论 (**伯特兰悖论**).
  - 其中一个悖论涉及初等概率论 ([李永乐视频链接](https://www.ixigua.com/7158761116787704354)) 
  - 另一个悖论就是我们即将介绍的博弈论模型.
...
伯特兰模型中, 张三和李四的**效用函数**通过如下方式计算:
- 需求函数为严格递减函数 $Q(p)$, 其中市场价格 $p$ 为 $p_1$ 和 $p_2$ 中的较小值: 
  - $p = \min \{p_1, p_2\}$
- 若张三定价 $p_1$ 高于李四定价 $p_2$, 则张三垄断整个市场并获得垄断利润,
  李四的利润为零.  
...
**问:** 能否套用之前古诺模型("同时定产模型")中的方法, 结合最优反应函数来计算伯特兰模型("同时定价模型")的纳什均衡? 

**答:** 不能. 给定李四的定价 $p_2 > c$, 张三的利润  $\pi(p_1)$ 为:
$$
\pi(p_1) = \begin{cases}
...
- 厂商的成本不是外生给定的, 而是和厂商本身的研发投入有关. 并且,
  厂商的成本通常是自己的私人信息, 不为其它厂商所知.
- ...
